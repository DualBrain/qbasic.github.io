...
[CONST](CONST), DEFDBL, DEFINT, DEFLNG, DEFSNG, DEFSTR, DOUBLE, [HEX$](HEX$), INTEGER, [LET](LET), LONG, [OCT$](OCT$), SINGLE, STRING, SWAP, TYPE...END TYPE
...
DATA, DIM, $DYNAMIC, ERASE, LBOUND, [OPTION BASE](OPTION-BASE), READ, REDIM, RESTORE, $STATIC, UBOUND
...
[CLS](CLS), CSRLIN, INKEY$, INPUT, INPUT$, LINE INPUT, LOCATE, POS, [PRINT](PRINT), PRINT USING, [SPC](SPC), [TAB](TAB), VIEW PRINT, WIDTH, WRITE
...
CIRCLE, [COLOR](COLOR), DRAW, GET, LINE, PAINT, PALETTE, PCOPY, PMAP, POINT, [PRESET](PRESET), [PSET](PSET), PUT, [SCREEN](SCREEN), VIEW, WINDOW
...
CALL, CALL ABSOLUTE, CASE, [CDBL](CDBL), CHAIN, [CHDIR](CHDIR), [CHR$](CHR$), [CINT](CINT), CIRCLE, [CLEAR](CLEAR), [CLNG](CLNG), CLOSE, [CLS](CLS), [COLOR](COLOR), COM(n), COMMON, [CONST](CONST), [COS](COS), [CSNG](CSNG), CSRLIN, CVD, CVDMBF, CVI, CVL, CVS, CVSMBF

### D

DATA, DATE$, DECLARE, [DEF FN](DEF-FN), DEF SEG, DEFDBL, DEFINT, DEFLNG, DEFSNG, DEFSTR, DIM, [DO...LOOP](DO...LOOP), [DO UNTIL](DO-UNTIL), [DO WHILE](DO-WHILE), DOUBLE, DRAW
...
LBOUND, [LCASE$](LCASE$), [LEFT$](LEFT$), [LEN](LEN), [LET](LET), LINE, LINE INPUT, LINE INPUT#, LOC, [LOCATE](LOCATE), LOCK, LOF, [LOG](LOG), LONG, LOOP, LPOS, LPRINT, LPRINT USING, LSET, [LTRIM$](LTRIM$)
...
RANDOM, RANDOMIZE, READ, REDIM, [REM](REM), RESET, RESTORE, RESUME, [RETURN](RETURN), [RIGHT$](RIGHT$), [RMDIR](RMDIR), [RND](RND), RSET, [RTRIM$](RTRIM$), [RUN](RUN)

### S

[SCREEN](SCREEN), SCREEN Function, SEEK, SEEK (Statement), SELECT, [SGN](SGN), SHARED, SHELL, [SIN](SIN), SINGLE, SLEEP, SOUND, [SPACE$](SPACE$), [SPC](SPC), [SQR](SQR), STATIC, [STEP](STEP), STICK, [STOP](STOP), [STR$](STR$), STRIG, STRIG(n), STRING, [STRING$](STRING$), SUB, SWAP, [SYSTEM](SYSTEM)
